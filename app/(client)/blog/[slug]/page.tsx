          ...
          <div>
            <div className="text-xs flex items-center gap-5 my-7">
              ...
              <p className="flex items-center gap-1 text-lightColor relative group hover:cursor-pointer hover:text-indigo-800 hoverEffect">
                ...
                <span className="absolute left-0 -bottom-1.5 bg-lightColor/30 inline-block w-full h-[2px] group-hover:bg-indigo-800 hoverEffect" />
              </p>
              <p className="flex items-center gap-1 text-lightColor relative group hover:cursor-pointer hover:text-indigo-800 hoverEffect">
                <Calendar size={15} />{" "}
                  ...
                  {blog.body && (
                    <PortableText
                      ...
                      components={{
                        block: {
                          normal: ({ children }) => (
                            <p className="my-5 first:mt-0 last:mb-0 text-xl">
                              {children}
                            </p>
                          ),
                          h2: ({ children }) => (
                            <h2 className="my-5 text-3xl/8 font-medium tracking-tight text-gray-950 first:mt-0 last:mb-0">
                              {children}
                            </h2>
                          ),
                          h3: ({ children }) => (
                            <h3 className="my-5 text-2xl/8 font-medium tracking-tight text-gray-950 first:mt-0 last:mb-0">
                              {children}
                            </h3>
                          ),
                          blockquote: ({ children }) => (
                            <blockquote className="my-5 border-l-2 border-l-gray-300 pl-6 text-2xl/8 text-gray-950 first:mt-0 last:mb-0">
                              {children}
                            </blockquote>
                          ),
                        },
...
const BlogLeft = async ({ slug }: { slug: string }) => {
  ...
  const blogs = await getOthersBlog(slug, 9);
            ...
            >
              {blog?.mainImage && (
                <Image
                  ...
                  width={100}
                  height={100}
                  className="w-16 h-16 rounded-full object-cover border-[1px] border-indigo-500/10 group-hover:border-indigo-500 hoverEffect"
                />
              )}
              <p className="line-clamp-2 text-sm text-lightColor group-hover:text-indigo-500 hoverEffect">
                {blog?.title}
              </p>
            </Link>
          ))}
